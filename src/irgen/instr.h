...
public:
    FuncInstr(type_ptr_t retType, std::vector<use_ptr_t> args, std::list<block_ptr_t> blocks, std::string name = "")
        : User(nullptr, std::move(name)), retType(std::move(retType)), args(std::move(args)), blocks(std::move(blocks)) {}
    ~FuncInstr() = default;
    type_ptr_t getRetType() { return retType; }
    const std::vector<use_ptr_t> &getArgs() { return args; }
    void addBlock(block_ptr_t block) { blocks.emplace_back(std::move(block), this); }
...
public:
    explicit NegInstr(value_ptr_t from) : from(make_use(std::move(from), this)) {}
    ~NegInstr() = default;
    std::string dump() const override;
};
...
public:
    CmpInstr(use_ptr_t lhs, use_ptr_t rhs, CompareType cmpType)
        : lhs(make_use(std::move(lhs), this)), rhs(make_use(std::move(rhs), this)), cmpType(cmpType) {}
    ~CmpInstr() = default;
    std::string dump() const override;
};
...
public:
    ...
    ~InstrBlock() = default;
    void addInstr(user_ptr_t instr) { instrs.emplace_back(std::move(instr), this); }
    std::string dump() const override;
};
...
public:
    Program() = default;
    ~Program() = default;
    void addFunc(use_ptr_t func) { funcs.emplace_back(std::move(func), this); }
    std::string dump() const override;
};