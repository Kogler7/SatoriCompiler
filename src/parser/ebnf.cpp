...
vector<production> EBNFParser::segmentProduct(production product)
{
    ...
    while (lastIt != right.end())
    {
        ...
        else
        {
            ...
            if (deliEndIt == right.end())
            {
                error << "EBNFParser: EBNF syntax error: Expected " << endDeli << ", got EOF";
...
vector<production> EBNFParser::geneStxProducts(token_iter start, token_iter end)
{
    vector<production> products;
    ...
    for (auto it = start; it != end; it++)
    {
        ...
        {
            error << "EBNFParser: EBNF syntax error: Expected ::=, got " << it->value;
            cout << " at <" << it->line << ", " << it->col << ">" << endl;
            exit(1);
        }
        it++;
...
vector<production> EBNFParser::geneMapProducts(token_iter start, token_iter end)
{
    vector<production> products;
    ...
    for (auto it = start; it != end; it++)
    {
        if (it->type != mulTermType)
        {
            error << "EBNFParser: MAPPING syntax error: Expected mul-terminal, got " << it->value;
            cout << " at <" << it->line << ", " << it->col << ">" << endl;
            exit(1);
        }
        term left = it->value;
        ...
        {
            error << "EBNFParser: MAPPING syntax error: Expected ->, got " << it->value;
            cout << " at <" << it->line << ", " << it->col << ">" << endl;
            exit(1);
        }
        it++;
...
void EBNFParser::addRules(vector<production> &products)
{
    token_type nonTermType = get_tok_type("NON_TERM");
...
void EBNFParser::addMappings(vector<production> &products)
{
    ...
    for (auto &pro : products)
    {
        ...
        if (right.size() != 1)
        {
            error << "EBNFParser: Mapping syntax error: Expected only one token-type term, got " << right.size();
            cout << " at <" << right[0].line << ", " << right[0].col << ">" << endl;
            exit(1);
        }
        else if (right[0].type != tokType)
        {
...
Grammar EBNFParser::parse(string grammarPath)
{
    ...
    vector<production> products = geneStxProducts(blkStart + 1, blkEnd);
    addRules(products);
    // 解析MAPPING映射
